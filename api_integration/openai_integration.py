...
def alt_recipe_query(key, diet, recipe_query):
    ...
    completion = client.chat.completions.create(
        ...
        messages=[
            {"role": "system", "content":
                ...
                "You return the prompt with the alternate recipe name in quotes"
                f"I am looking to make the following recipe, with alternative"
    ...
    def parse_alternative_recipe_string(recipe_string):
        ...
        ingredients_start = recipe_string.find("Ingredients:") + len("Ingredients:")
        ingredients_end = recipe_string.find("Instructions:")

        if ingredients_start != -1 and ingredients_end != -1:
            ingredients_section = recipe_string[ingredients_start:ingredients_end].strip()
